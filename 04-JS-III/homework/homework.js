...
//return arreglo.reduce((contador, elemento) => elemento > 18 ? contador + 1 : contador , 0 )
}
...
function diaDeLaSemana(numeroDeDia) {
 ...
 /*
  return numeroDeDiaia === 1 || numeroDeDia === 7 ? "Es fin de semana" : "Es dia Laboral";
  */
} 
...
function todosIguales(arreglo) {
  ...
  let primElemento=arreglo[0];
  for (let index = 1; index < arreglo.length; index++) {
    if (arreglo[index]!==primElemento) {
      return false;
...
function mayorACien(array) {
  ...
  // Tu código: return  array.filter(x => x > 100);
  mayorACien= [];
  for (let index = 0; index < array.length; index++) {
   if (array[index]>100) {
    mayorACien.push(array[index]);//si es mayor a 100 se agrega a resultado utilizando push().
   }
   return mayorACien;
  }
}
